...
get_Arrays <- function(crispr) {
    ...
    Arrays <- lapply(crispr_id, \(i){
        ...
        res <- as.list(tmp)
        spacers <- crispr$Spacer %>%
            dplyr::filter(CRISPR_id == i) %>%
            dplyr::pull(sequence)
...
calculate_identity <- function(s1, s2, xmode = FALSE) {
    ...
    if (xmode) {
        ...
        flag <- tryCatch(expr = {
            dna_s1 <- Biostrings::DNAString(s1)
            TRUE
        }, error = function(e) {
            FALSE
        })
...
compare_array <- function(array1, array2, identity = TRUE) {
    ...
    array_spacer <- rbind(
        data.frame(id = array1_id, spacer = array1, array = "array1", position = seq_along(array1)),
    ...
    if (identity) {
        ...
        compare$identity <- apply(compare, 1, \(i)max(is_same_seq(i[3], i[6], xmode = TRUE)))
    }

    compare <- compare %>% mutate(Identity_level = cut(identity,
    ...
    if (nrow(links) > 0) {
        ...
        array_spacer$membership <- paste0("M", seq_len(nrow(array_spacer)))
    }
...
plot.array_comparison <- function(x, linewidth = 2, ...) {
    ...
    if (nrow(links) > 0) {
        p <- p + geom_segment(
            data = dplyr::filter(compare, identity >= 0.8),
    ...
    if (any(duplicated(array_spacer$membership))) {
        ...
        p <- p + geom_tile(data = array_spacer, mapping = aes(x = position, y = array), width = 0.55, height = 0.25, fill = "#78c679")
    }

    if (!is.null(x$array_align_res)) {
        sv_res <- summary_SV(x$array_align_res)
        title <- paste0(
            "Length: ", sv_res$Length, "; Match: ", sv_res$Match, "; Translocation: ",
            sv_res$Translocation, "\nDeletion: ", sv_res$Deletion, "; Insertion: ", sv_res$Insertion
        )
        p <- p + labs(title = title)
    }
    if (max(array_spacer$position) > 20) {
        p <- p + coord_fixed(ratio = 1.5)
    } else if (max(array_spacer$position) > 40) {
...
summary_SV <- function(align_res) {
    ...
    return(list(
        align1 = align_res$align1, align2 = align_res$align2, SV_index = SV_index, Length = Length,
...
install_mafft <- function(mafft.zip = NULL, install_dir = tools::R_user_dir("iCRISPR")) {
    ...
    set_iCRISPR_config("MAFFT", paste0(install_dir, "/mafft-mac/mafft.bat"))
    message("MAFFT installed successfully at", install_path, "\n")
}
